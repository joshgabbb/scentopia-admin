// app/api/admin/analytics/heatmap/orders/route.ts
// Returns orders that were delivered to a specific province

import { createClient } from "@/lib/supabase/server";
import { NextRequest } from "next/server";

const provinceCoordinates: Record<string, { lat: [number, number]; lng: [number, number] }> = {
...
function getProvinceFromCoordinates(lat: number, lng: number): string | null {
  for (const [code, bounds] of Object.entries(provinceCoordinates)) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!provinceCode) {
      return Response.json({ error: "province param is required" }, { status: 400 });
    }
    ...
    const matched = (orders || []).filter((order) => {
      ...
      return getProvinceFromCoordinates(lat, lng) === provinceCode;
    });

    const formatted = matched.map((order) => {
    ...
    return Response.json({ success: true, orders: formatted, total: formatted.length });
  } catch (err) {
    console.error("Heatmap orders error:", err);
    return Response.json({ error: "Failed to fetch province orders" }, { status: 500 });
  }
}
