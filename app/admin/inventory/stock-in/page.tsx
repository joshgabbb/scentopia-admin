...
import { ArrowDownToLine, CheckCircle, AlertCircle, Loader2, Search, X, Package, ChevronDown } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast("success", `Added ${qty} perfume${qty !== 1 ? "s" : ""} to ${selectedSize}. New stock: ${result.newStock} perfumes.`);
        setQuantity("");
        setRemarks("");
        setSelectedSize("");
        setReason("");
          ...
          {selectedProduct && (
            <p className="text-xs text-green-700 bg-green-50 border border-green-200 rounded px-2 py-1 mt-2">
              ...
              {selectedProduct.category && (
                <span className="text-green-600 ml-1">· {selectedProduct.category.name}</span>
              )}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          >
            ...
            {selectedProduct && Object.entries(selectedProduct.sizes ?? {}).map(([size]) => {
              ...
              return (
                <option key={size} value={size}>{size} — {stock} perfumes in stock</option>
              );
            })}
          </select>
          {currentSizeStock !== null && (
            <p className="text-xs text-[#7a6a4a] mt-1.5">
              Current stock: <span className="font-semibold text-[#1c1810]">{currentSizeStock} perfumes</span>
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {selectedSize && currentSizeStock !== null && qty > 0 && (
            <p className="text-xs text-green-600 mt-1.5 font-medium">
              New stock will be: {currentSizeStock + qty} perfumes
            </p>
          )}
        </div>
