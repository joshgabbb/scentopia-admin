...
function StockInModal({ alert, onClose, onSuccess }: StockInModalProps) {
  ...
  // Fetch product sizes from /api/admin/products/[id]
  useEffect(() => {
    const fetchSizes = async () => {
      try {
        const res = await fetch(`/api/admin/products/${alert.productId}`);
        const result = await res.json();
        if (result.success && result.data?.sizes) {
          const sizeKeys = Object.keys(result.data.sizes as Record<string, unknown>);
          setSizes(sizeKeys);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await res.json();
      if (!result.success) { setError(result.error || "Failed to stock in."); return; }
      onSuccess();
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white border border-[#e8e0d0] shadow-xl w-full max-w-md">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8e0d0] bg-[#faf8f3]">
          <div className="flex items-center gap-2">
            <ArrowDownToLine className="w-4 h-4 text-[#d4af37]" />
            <h2 className="font-semibold text-[#1c1810]">Stock In</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {/* Product (read-only) */}
          <div>
            <label className="block text-xs font-medium text-[#7a6a4a] uppercase tracking-wide mb-1">Product</label>
            <div className="px-3 py-2 bg-[#faf8f3] border border-[#e8e0d0] text-sm text-[#1c1810] font-medium">
              {alert.productName}
            </div>
            <div className="mt-1 text-xs text-[#9a8a6a]">
              Current stock: <span className="font-medium text-[#1c1810]">{alert.currentStock} units</span>
            </div>
          </div>

          {/* Size */}
          <div>
            <label className="block text-xs font-medium text-[#7a6a4a] uppercase tracking-wide mb-1">Size</label>
            {loadingSizes ? (
              <div className="h-9 bg-[#faf8f3] border border-[#e8e0d0] animate-pulse" />
            ) : sizes.length > 0 ? (
              <select
                value={selectedSize}
                onChange={(e) => setSelectedSize(e.target.value)}
                className="w-full px-3 py-2 border border-[#e8e0d0] text-sm text-[#1c1810] bg-white focus:outline-none focus:ring-2 focus:ring-[#d4af37]"
                required
              >
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-[#e8e0d0] text-sm text-[#1c1810] focus:outline-none focus:ring-2 focus:ring-[#d4af37]"
              />
            )}
          </div>
          ...
          {/* Quantity */}
          <div>
            <label className="block text-xs font-medium text-[#7a6a4a] uppercase tracking-wide mb-1">Quantity to Add</label>
            <input
              type="number"
              min="1"
              value={quantity}
          ...
          {/* Reason */}
          <div>
            <label className="block text-xs font-medium text-[#7a6a4a] uppercase tracking-wide mb-1">Reason</label>
            <select
              value={reason}
          ...
          {/* Remarks */}
          <div>
            <label className="block text-xs font-medium text-[#7a6a4a] uppercase tracking-wide mb-1">Remarks <span className="normal-case font-normal">(optional)</span></label>
            <input
              type="text"
              value={remarks}
          ...
          <div className="flex gap-2 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 border border-[#e8e0d0] text-sm text-[#7a6a4a] hover:bg-[#faf8f3] transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 px-4 py-2 bg-[#d4af37] text-[#0a0a0a] text-sm font-semibold hover:bg-[#c4a030] disabled:opacity-50 transition-colors"
            >
...
export default function InventoryAlertsPage() {
  ...
  const handleStockInSuccess = () => {
    const name = stockInAlert?.productName ?? "Product";
    setStockInAlert(null);
    setStockInToast(`Stock In recorded for ${name}. Refreshing…`);
    fetchAlerts();
    setTimeout(() => setStockInToast(""), 4000);
  };
  ...
  return (
    <>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Recommendation + Stock In action */}
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {alert.type !== "slow_moving" && (
                        <button
                          onClick={() => setStockInAlert(alert)}
                          className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#d4af37] text-[#0a0a0a] text-xs font-semibold hover:bg-[#c4a030] transition-colors"
                        >
      ...
      {stockInAlert && (
        <StockInModal
          alert={stockInAlert}
          onClose={() => setStockInAlert(null)}
          onSuccess={handleStockInSuccess}
        />
      )}

      {/* Stock In Success Toast */}
      {stockInToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-emerald-600 text-white px-4 py-3 shadow-lg text-sm flex items-center gap-2">
