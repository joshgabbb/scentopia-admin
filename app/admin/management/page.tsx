...
export default function ManagementPage() {
  ...
  const modules = [
    ...
    },
    {
      ...
      path: "/admin/management/inventory-alerts",
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-[#faf8f3] border border-[#e8e0d0] p-6">
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-[#7a6a4a]">
          Manage feedback, notifications, audit trails, and inventory alerts
        </p>
      </div>
      ...
      {stats.lowStockAlerts > 0 && (
        <div className="bg-red-900/20 border border-red-500/30 p-4 flex items-start gap-3">
          ...
          <button
            onClick={() => handleModuleClick('/admin/management/inventory-alerts')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {modules.map((module) => (
          <button
            ...
            className="bg-[#faf8f3] border border-[#e8e0d0] p-6 hover:border-[#d4af37]/50 transition-all duration-200 text-left group"
          >
            <div className="flex items-start justify-between mb-4">
              <div className="bg-white border border-[#e8e0d0] p-3 text-2xl">
                {module.icon}
              </div>
            ...
            <h3 className="text-lg font-semibold text-[#1c1810] mb-2 group-hover:text-[#d4af37]">
              {module.title}
            </h3>
            <p className="text-[#7a6a4a] text-sm mb-4">{module.description}</p>

            <div className="flex items-center justify-between">
              <span className="text-xs text-[#7a6a4a]">{module.trendValue}</span>
      ...
      <div className="bg-[#faf8f3] border border-[#e8e0d0] p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center bg-white border border-[#e8e0d0] p-4">
            <div className="text-2xl font-bold text-[#d4af37]">{stats.pendingFeedback}</div>
            <div className="text-xs text-[#7a6a4a] mt-1">Pending Feedback</div>
          </div>
          <div className="text-center bg-white border border-[#e8e0d0] p-4">
            <div className="text-2xl font-bold text-red-500">{stats.lowStockAlerts}</div>
            <div className="text-xs text-[#7a6a4a] mt-1">Inventory Alerts</div>
          </div>
          <div className="text-center bg-white border border-[#e8e0d0] p-4">
            <div className="text-2xl font-bold text-[#d4af37]">{stats.pendingNotifications}</div>
            <div className="text-xs text-[#7a6a4a] mt-1">Notifications</div>
          </div>
          <div className="text-center bg-white border border-[#e8e0d0] p-4">
            <div className="text-2xl font-bold text-[#d4af37]">{stats.auditLogs}</div>
            <div className="text-xs text-[#7a6a4a] mt-1">Audit Logs (30d)</div>
          </div>
        </div>
      </div>
    </div>
