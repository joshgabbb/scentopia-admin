    ...
    <div className="bg-[#faf8f3] border border-[#e8e0d0] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {loading && (
            <span className="text-sm text-[#7a6a4a]">Loading data...</span>
          )}
          {error && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Map Section */}
        <div className="flex flex-1 h-[600px] flex-col justify-center items-center w-full bg-white border border-[#e8e0d0] py-8">
        ...
        {/* Details Section */}
        <div className="flex flex-1 h-[600px] flex-col justify-start items-start w-full bg-white border border-[#e8e0d0] px-5 py-4 overflow-y-auto">
          <div className="w-full space-y-4">
            ...
            <div>
              ...
              <label className="block text-sm font-medium text-[#7a6a4a] mb-2 mt-4">
                Select Region
              </label>
              <select
            ...
            {/* Legend */}
            <div>
              <h3 className="text-sm font-medium text-[#7a6a4a] mb-2">Legend (Orders):</h3>
              <div className="flex flex-col space-y-2">
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-green-500"></div>
                  <span className="text-sm text-[#1c1810]">Low (0-{statistics.lowThreshold})</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-orange-500"></div>
                  <span className="text-sm text-[#1c1810]">Medium ({statistics.lowThreshold + 1}-{statistics.highThreshold})</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-red-500"></div>
                  <span className="text-sm text-[#1c1810]">High ({statistics.highThreshold + 1}+)</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-[250px] overflow-y-auto">
                {sortedData.length > 0 ? (
                  sortedData.map(([id, value]) => (
                    <div key={id} className="flex items-center justify-between p-2 bg-[#faf8f3] border border-[#e8e0d0] text-sm">
                      <div className="flex items-center space-x-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: getColorForValue(value) }}
                        ></div>
                        <div className="font-medium text-[#1c1810]">
                          {provinceNames[id] || id}
                        </div>
                      </div>
                  ...
                  <div className="text-center text-[#7a6a4a] mt-8">
