...
function ProductThumb({
  ...
  return (
    <div
      className={`${dim} bg-[#faf8f3] border border-[#e8e0d0] flex-shrink-0 overflow-hidden rounded-sm`}
    >
      {product.image ? (
        <img
          src={product.image}
      ...
      <div className="bg-[#faf8f3] border border-[#e8e0d0] p-6">
        ...
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-[#7a6a4a]">Analyzing customer purchases...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-[#faf8f3] border border-[#e8e0d0] p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <select
            ...
            className="px-3 py-2 bg-white border border-[#e8e0d0] text-[#1c1810] text-sm focus:outline-none focus:ring-2 focus:ring-[#d4af37]"
          >
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-2 bg-white border border-[#e8e0d0] px-4 py-2 rounded-full text-sm">
          <span className="text-[#7a6a4a]">Orders analysed:</span>
          <span className="font-semibold text-[#1c1810]">
            {data.summary.totalOrders.toLocaleString()}
          </span>
        </div>
        <div className="flex items-center gap-2 bg-white border border-[#e8e0d0] px-4 py-2 rounded-full text-sm">
          <span className="text-[#7a6a4a]">Product pairs found:</span>
          <span className="font-semibold text-[#1c1810]">
            {data.summary.rulesFound}
          </span>
        </div>
        <div className="flex items-center gap-2 bg-white border border-[#e8e0d0] px-4 py-2 rounded-full text-sm">
          <span className="text-[#7a6a4a]">Avg items per order:</span>
          <span className="font-semibold text-[#1c1810]">
      ...
      {activeTab === "patterns" && (
        <div className="space-y-4">
          {/* Intro */}
          <div className="bg-white border border-[#e8e0d0] px-4 py-3 rounded-sm text-sm text-[#1c1810]">
          ...
          {data.associationRules.length === 0 ? (
            <div className="text-center py-14 bg-white border border-[#e8e0d0]">
              <div className="text-4xl mb-3">🛒</div>
              <p className="text-[#d4af37] font-semibold text-lg">No patterns found yet</p>
              <p className="text-[#7a6a4a] text-sm mt-1">
                ...
                .map((rule, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-[#1c1810] truncate">
                        ...
                        <div className="flex flex-col items-center flex-shrink-0 px-1">
                          ...
                          <div className="text-[10px] text-[#7a6a4a] mt-0.5 whitespace-nowrap">
                            bought together
                          </div>
                        </div>
                      ...
                      {/* Stats row */}
                      <div className="mt-3 pt-3 border-t border-[#e8e0d0] flex flex-wrap items-center gap-3">
                        {/* Strength badge */}
                        <span
                      ...
                      <div className="mt-2 text-xs text-[#7a6a4a] italic">
                        {getPairNote(rule)}
                      </div>
                    </div>
                  );
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-[10px] font-semibold text-green-600 uppercase tracking-wide mb-0.5">
                              Best seller
                            </div>
                            <p className="text-sm font-medium text-[#1c1810] truncate">
                      ...
                      {/* Price block */}
                      <div className="flex items-center gap-5 lg:border-l lg:border-[#e8e0d0] lg:pl-5 flex-shrink-0">
                        <div>
                          <p className="text-xs text-[#7a6a4a]">Bundle price</p>
                        ...
                        <div>
                          <p className="text-xs text-[#7a6a4a]">Est. revenue</p>
                          <p className="text-xl font-bold text-green-600">
                            {formatCurrency(bundle.potentialRevenue)}
                          </p>
                          <p className="text-xs text-[#7a6a4a]">if promoted</p>
                        </div>
                      </div>
                    </div>

                    {/* Pattern badge + reasoning */}
                    <div className="mt-3 pt-3 border-t border-[#e8e0d0] flex flex-wrap items-start gap-2">
                      {patternStrength ? (
                        <span
                        ...
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#faf8f3] text-[#7a6a4a]">
                          New pairing suggestion
                        </span>
                      )}
                      <p className="text-xs text-[#7a6a4a] flex-1">
                        {bundle.reasoning}
                      </p>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-[#e8e0d0]">
            <div className="px-4 py-3 border-b border-[#e8e0d0]">
              <h3 className="font-semibold text-green-700">Top Sellers</h3>
            ...
            <div className="divide-y divide-[#e8e0d0]">
              {data.topSellers.length === 0 ? (
                <p className="px-4 py-6 text-sm text-[#7a6a4a] text-center">
                  No sales data yet
                </p>
              ) : (
          ...
          <div className="bg-white border border-[#e8e0d0]">
            <div className="px-4 py-3 border-b border-[#e8e0d0]">
              <h3 className="font-semibold text-amber-700">Needs Attention</h3>
              <p className="text-xs text-[#7a6a4a] mt-0.5">
